...
# isfrr_ready checks that the frr container is running and staticd is active.
...
isfrr_ready() {
    ...
    if echo "$daemons" | grep -q "staticd"; then
        echo "frr is ready (staticd running)"
        return 0
    else
        echo "frr is running but staticd not active" >&2
        return 1
    fi
}
