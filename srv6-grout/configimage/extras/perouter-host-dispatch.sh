...
# Look up per-node underlay addresses from openperouter.env
read -r _ UNDERLAY_V4 UNDERLAY_V6 <<< \
	"$(echo "$UNDERLAY_ADDRESSES" | grep -w "^${NODE_NAME}")"
if [ -z "$UNDERLAY_V4" ] || [ -z "$UNDERLAY_V6" ]; then
	die "no UNDERLAY_ADDRESSES entry for $NODE_NAME in openperouter.env"
fi
